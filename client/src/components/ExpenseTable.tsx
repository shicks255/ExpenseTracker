...
import { ArrowDown, ArrowDownUp, ArrowUp, CalendarIcon, Search, X } from 'lucide-react';
...
export default function ExpenseTable() {
    ...
    useOnClickOutside(floatingRef, () => setDatePickerOpen(false));
    ...
    const changeDateFilter = (value: string) => {
        ...
        // Implement logic to update the date filter based on the selected value
    };
        ...
        <>
            <div className='relative inline-block'>
                ...
                <Select value={dateFilter} onValueChange={changeDateFilter}>
                    ...
                    <SelectContent>
                        <SelectGroup>
                            ...
                            <SelectItem value="pastFullMonth">Last full month</SelectItem>
                ...
                {datePickerOpen && (
                    <div
                        ref={floatingRef}
                        className="absolute left-0 top-full z-50 mt-2 rounded-md border bg-background p-3 shadow-md"
                    >
                        <Calendar
                            mode="range"
                ...
                <Field>
                    ...
                    <InputGroup className="max-w-xs" onBlur={() => setVendorParam(vendorFilter)}>
                        <InputGroupInput onChange={(e) => setVendorFilter(e.target.value)} value={vendorFilter} placeholder="Search..." />
                        <InputGroupAddon>
                            <Search />
                        </InputGroupAddon>
                ...
                <TableBody>
                    {expenses.data.map((expense) => (
                        <TableRow key={expense.id}>
                            ...
                            <TableCell>{expense.category_id && categories.data.find((cat) => cat.id === expense.category_id)?.name}</TableCell>
                            <TableCell>{expense.note}</TableCell>
                            <TableCell className="text-right">${Number(expense.amount / 100).toFixed(2)}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
                <TableFooter>
                    <TableRow>
                        ...
                        <TableCell className="text-right">
                            ${expenses.data.reduce((sum, expense) => sum + Number(expense.amount) / 100, 0).toFixed(2)}
                        </TableCell>
                    </TableRow>
                </TableFooter>
            </Table >
